...
nocite: |
  @*
suppress-bibliography: true  # Add this line
execute:
  echo: false
  warning: false
  message: false
...
fmt_authors_card <- function(entry) {
  ...
  author_names <- vapply(au, function(a) {
    fam_vec <- a$family
    if (length(fam_vec)) paste(as.character(fam_vec), collapse = " ") else ""
  }, character(1))
  
  author_names <- author_names[nzchar(author_names)]
  if (length(author_names) == 0) return(NA_character_)
  
  # Bold the first author
  if (length(author_names) > 0) {
    author_names[1] <- paste0("<strong>", author_names[1], "</strong>")
  }
  ...
  if (length(author_names) == 1) {
    ...
    paste(
      paste(author_names[-length(author_names)], collapse = ", "),
      "&",
